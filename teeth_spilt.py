...
def main():
    # 使用 Tkinter 創建文件對話框來選擇資料夾
    root = tk.Tk()
    root.withdraw()
    desktop_path = filedialog.askdirectory(title="Choose the image folder")
    ...
    gum_path = os.path.join(color_path, "gum")
    teeth_path = os.path.join(color_path, "teeth")
    dentalcrown_path = os.path.join(color_path, "dentalcrown")
    dentin_path = os.path.join(color_path, "Dentin")
    bw1_path = os.path.join(bw_path, "gum_bw")
    bw2_path = os.path.join(bw_path, "teeth_bw")
    bw3_path = os.path.join(bw_path, "dentalcrown_bw")
    bw4_path = os.path.join(bw_path, "dentin_bw")
